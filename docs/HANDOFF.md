...
- 去重依赖的日志形状（一条消息多行、每行带 `message.id` 和 `requestId`）来自 ccusage 等工具的公开做法，写的时候**没有拿本机真实
  日志核对**。**2026-09-24 在本机核对了，前提成立**：497 个日志文件里，同一个
  `(message.id, requestId)` 平均占 2.26 行，重复行的 `usage` 逐字段相同。改前改后见
  `MAC_HANDOFF_2026-09-23.md` 末尾的表：回合 −55.8%、等价成本 −59.3%，54 天里没有一天变成 0。
...
- **FSEvents / kqueue 两个监听器**（2026-09-23）在 CI 的真内核上验证过会触发；2026-09-24 又把 `TreeWatcher` 单独接到本机
  真实的 `~/.claude/projects` 和 `~/.codex/sessions` 上跑了 45 秒，首次 `drain()` 按设计返回 `.unknown`，之后每 3 秒
  都报到了正在写的会话文件（共 24 个路径事件），路径拼写与调用方一致。**仍然没有在 app 里长时间跑过。**
