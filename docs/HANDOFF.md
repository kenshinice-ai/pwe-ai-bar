...
- **~~面板比屏幕高~~（1.0.1 修）**。八家全开 + full 模式下面板是 913 pt，1440×900 的机器放不下，
  AppKit 把溢出的部分推到屏幕上方——标题栏、来源选择器、续航仪全都够不着。现在标题栏和页脚
  钉住，中间滚动，高度上限 `PanelView.ceiling`（跟随屏幕，最多 860 pt）。设置页早就因为
  同样的原因加过滚动，面板漏了。`PanelHeightTests` 守着。
