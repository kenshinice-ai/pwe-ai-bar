...
- **~~面板比屏幕高~~（1.0.1 修，1.0.2 改对）**。八家全开 + full 模式下面板是 913 pt，
  1440×900 的机器放不下，AppKit 把溢出的部分推到屏幕上方——标题栏、来源选择器、续航仪
  全都够不着。现在标题栏和页脚钉住，中间滚动，上限 `PanelView.ceiling(usableHeight:)`。
  **上限只跟屏幕走，不要再加一个绝对值。** 1.0.1 里我顺手又压了个 860 pt，理由是
  「再高就不像菜单栏面板了」——那是审美不是约束，在 1334 pt 的屏幕上白白扔掉 474 pt，
  把一个「够不着」的修复变成了一条没人需要的滚动条，还把一行截成两半。
  内容自己到 913 pt 就封顶，所以正常屏幕上根本不会出现滚动条。
  `PanelHeightTests` 两个方向各守一个：小屏要截断且可滚，大屏要一次全显示。
