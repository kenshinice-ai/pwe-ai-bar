...
- **~~面板比屏幕高~~（1.0.1 修，1.0.2 修歪，1.0.3 才修对）**。原问题是真的：八家全开 + full
  模式下内容 913 pt，1440×900 放不下，AppKit 把溢出部分推到屏幕上方，标题栏和续航仪够不着。

  **但 1.0.1 的修法是错的，而且错得不显眼。** 无条件包一层 `ScrollView` 之后，
  **面板不再向 popover 要任何高度**——`ScrollView` 在纵向是完全可伸缩的，不产生约束。
  于是 popover 保持它碰巧有的尺寸（约 300 pt），整个面板在里面滚，一行被从中间截断。
  1.0.2 把上限从 860 提到跟随屏幕，毫无作用，因为**上限从来就没被碰到过**。

  量出来的：一根普通的列能把一个 300 pt 的窗口顶开到它需要的 913；同一根列包进
  `ScrollView` 之后，窗口塌到 76——只剩钉住的标题栏和页脚。

  1.0.3 的做法：先量内容（`MiddleHeight` / `ChromeHeight` 两个 PreferenceKey），
  **装得下就不要 ScrollView**（普通的列会把 popover 顶开），装不下才包，
  并且给它一个**固定高度**——固定高度是约束，popover 认。

  `PanelHeightTests` 因此改成把视图放进真实 `NSWindow`、跑 run loop、
  断言**窗口最后开到多大**。旧测试只问 `NSHostingView.fittingSize`，
  而那个值两种写法都回 913——它分不出「坚持自己高度的布局」和「给多少要多少的布局」，
  所以它眼睁睁放过了两个版本。
