...
//
// Scans every `L("key", "English")` call in Sources/, writes `Sources/PWEAIBar/Resources/en.lproj`
// from what it finds, and checks `zh-Hans.lproj` against it. Exits non-zero on any key that is used
...
//
// This app was written Chinese-first, so English is the fallback written at each call site and
// zh-Hans is the table maintained by hand — the same direction as the rest of the family.
