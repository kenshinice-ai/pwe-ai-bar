...
<sub>A macOS menu-bar meter for AI coding quota — Claude Code and Codex read from the real
endpoints, plus five more. It answers one question: *at this pace, do you reach the reset?*
**The interface is in Simplified Chinese only.** macOS 13+, Apple Silicon. Free.</sub>
...
- **它会续期过期的 Claude Code 令牌，而这条路径几乎没被真实触发过。**
  Claude Code 被实测到把凭据晾了 32 小时，所以应用会自己续，并把替代品写回同一个位置。
  那里有四条不变量和各自的回归测试，正是因为做错的代价是把你从你自己的 CLI 里登出。
  到目前为止，在真实账户上一次成功续期都还没有过。
